...
export class PlannerComponent {
  ...
  private buildPlan(debts: Debt[], strategy: Strategy, extraMonthly: number, maxMonths: number): PayoffPlan {
    ...
    for (let month = 1; month <= maxMonths; month++) {
      ...
      if (budgetLeft < -0.01) warning = 'Your budget is below the sum of minimum payments.';
    ...
    const paidOff = schedule.length && schedule[schedule.length - 1].remaining <= 0.01;
    if (!paidOff && !warning) warning = `Not paid off within ${maxMonths} months. Increase extra or max months.`;
